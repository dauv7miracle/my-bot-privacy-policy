...
app = Client("core", api_id=api_id, api_hash=api_hash)
...
def read_usernames():
    ...
    # Read the file and return non-empty, stripped lines
...
# 📢 Broadcast to contacts with quantity control and random delays
...
async def send_to_all_command(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            # Extract number from format like "!30" or "30"
            arg = message.command[1].lstrip('!')
            quantity = min(500, int(arg))  # Limit to 500 per run for safety
        except ValueError:
            pass
    ...
    if not all_usernames:
        await message.reply("❗ No usernames found in `01usernames.txt`.")
        return

    # Determine batch size
    total_count = len(all_usernames)
    if quantity is None:
        quantity = total_count  # Send to all if no quantity specified
    
    batch_size = min(quantity, total_count)
    status_msg = await message.reply(f"📤 Starting broadcast to {batch_size} of {total_count} users...")
    ...
    # Send messages with random delays and progress updates
    for i, username in enumerate(batch_usernames):
        try:
            # Send message copy (preserves formatting)
            ...
            if (i + 1) % 10 == 0 or (i + 1) == batch_size:
                await status_msg.edit_text(
                    f"📤 Broadcasting... {i+1}/{batch_size} "
                    f"({len(success)}✅ {len(failed)}❌)\n"
                    f"⏱ Next delay: {random.randint(5,10)}s"
                )
            
            # Random delay between 5-10 seconds (except after last message)
            if i < batch_size - 1:
            ...
            error_msg = str(e)
            failed.append(f"{username}: {error_msg}")
            # Remove invalid usernames from future runs
            if "USERNAME_INVALID" in error_msg or "USER_NOT_PARTICIPANT" in error_msg:
                remaining_usernames = [u for u in remaining_usernames if u != username]
    ...
    result = (
        ...
        f"📬 Success: {len(success)}/{batch_size}\n"
        f"❌ Failed: {len(failed)}/{batch_size}\n"
        f"📋 Remaining: {len(remaining_usernames)}\n"
        f"⏱ Avg delay: ~7.5s per message"
    )
    ...
    await status_msg.delete()  # Remove progress message
    await message.reply(result)
    await send_command_menu(message)

def read_usernames():
    """Read usernames from file with error handling"""
    try:
        with open("01usernames.txt", "r") as f:
