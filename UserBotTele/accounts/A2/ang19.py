...
app = Client("ang19", api_id=api_id, api_hash=api_hash)
...
async def send_to_all_command(client: Client, message: Message):
    ...
    # Copy list to avoid issues while removing
    usernames_copy = list(usernames)
    ...
    for username in usernames_copy:
        try:
            ...
            # Remove username after success
            usernames.remove(username)
        except Exception as e:
            failed.append(f"{username}: {e}")
            # Remove usernames that don't exist or cause errors
            if "username not found" in str(e).lower() or "peer_id_invalid" in str(e):
                usernames.remove(username)
        # Save current state after each attempt
    ...
    # Final save for remaining usernames
    with open("01usernames.txt", "w") as f:
        for user in usernames:
            f.write(user + "\n")
    ...
    if failed:
        result += "\n\n🔻 Failed usernames:\n" + "\n".join([f.split(":")[0] for f in failed[:10]])
        if len(failed) > 10:
